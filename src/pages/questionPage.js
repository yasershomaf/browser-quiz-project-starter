...
export const initQuestionPage = () => {
    ...
    for (const [key, answerText] of Object.entries(currentQuestion.answers)) {
        ...
        if (!currentStatus.userAnswers[currentStatus.currentIndex]) { //check that we didn't answer
            answerElement.addEventListener('click', checkAnswer)
        } else if (currentStatus.userAnswers[currentStatus.currentIndex].userAnswer === key) {
            answerElement.classList.add('red');
        } else if (currentStatus.userAnswers[currentStatus.currentIndex].correctAnswer === key) { //show answers that we did before
...
function checkAnswer() {
    ...
    if (this.textContent[5] === currentQuestion.correct) {
        ...
        setQuestionRecord('correct', quizData.currentQuestionIndex, currentQuestion.correct, currentQuestion.correct);
        ...
        setQuestionRecord('incorrect', quizData.currentQuestionIndex, currentQuestion.correct, this.textContent[5]);
...
function showAnswer() {
    ...
    const currentQuestion = quizData.questions[quizData.currentQuestionIndex];
    setQuestionRecord('incorrect', quizData.currentQuestionIndex, currentQuestion.correct);
    for (const answer of answersListElement) {
        if (answer.textContent[5] === currentQuestion.correct) {
            answer.classList.add('green');
        }
